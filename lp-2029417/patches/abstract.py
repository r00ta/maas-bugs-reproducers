...
class FileDescriptor(_ConsumerMixin, _LogOwner):
    ...
    def startWriting(self):
        ...
        # If the handler is closed the exception is raised all the times
        import os
        is_server = os.getenv("IS_SERVER", "nope")
        if is_server == "yes":
            if not self.trow:
                trow = input("Do you want asyncioreactor to raise builtins.RuntimeError? (y/n) ")
                if trow == 'y':
                    self.trow = True
            else:
                print("XXX It's not the first time an exception is raised. You should not be here as the connection should have "
                      "been dropped!")
            if self.trow:
                print("+ raising builtins.RuntimeError")
                # Simulating that self._asyncioEventloop.add_writer raise an exception
                import builtins
